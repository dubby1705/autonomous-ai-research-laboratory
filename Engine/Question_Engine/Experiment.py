...
MAX_PARALLEL_EXPERIMENTS = 3  # Parallel experiment design
...
def run_experiment_engine(
    ...
    def design_experiment_parallel(args):
        """Design a single experiment - runs in parallel"""
        i, hypothesis, domain_context = args
        try:
            local_client = Groq()
            design = design_experiment(local_client, hypothesis, domain_context)
            return (i, hypothesis, design, None)
        except Exception as e:
            return (i, hypothesis, None, str(e))
    
    print("\n  🔄 Designing experiments in parallel...")
    
    # Run experiment designs in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_PARALLEL_EXPERIMENTS) as executor:
        futures = {executor.submit(design_experiment_parallel, (i, hyp, domain_context)): (i, hyp) 
                   for i, hyp in enumerate(hypotheses, 1)}
        
        for future in concurrent.futures.as_completed(futures):
            i, hypothesis, design, error = future.result()
            
            if error:
                print(f"\n  [{i}/{len(hypotheses)}] ❌ Error: {error}")
                continue
            
            if not design:
                print(f"\n  [{i}/{len(hypotheses)}] ❌ Failed to design experiment.")
                continue
            
            # Step 2: Validate
            is_valid, issues = validate_experiment_design(design)
            if not is_valid:
                print(f"\n  [{i}/{len(hypotheses)}] ⚠️  Design has issues: {issues[:2]}")
            ...
            print(f"\n  [{i}/{len(hypotheses)}] ✅ {design.experiment_name}")
            print(f"      📊 {design.dataset} | 🎯 {design.primary_metric}")
