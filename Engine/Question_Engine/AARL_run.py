...
MAX_PARALLEL_LLM_CALLS = 4  # Parallel LLM calls
ENABLE_CACHING = True
DEEPENING_PARALLEL = True

# ==========================================
# AARL PIPELINE IMPORTS - OPTIMIZED
# ==========================================
...
def main():
    ...
    # --- PHASE 5: DEEPENING FEEDBACK LOOP (OPTIMIZED - Parallel Processing) ---
    if deepen_depth > 0 and approved_hypotheses:
        # Pre-load KB once for all hypotheses
        kb_items = []
        kb_loaded = False
        ...
        def process_hypothesis_deepening(hyp):
            """Process a single hypothesis for deepening - runs in parallel"""
            local_deepened = []
            ...
            current_items = list(set(relevant_items))
            ...
            for r in range(1, 5):
                ...
                for ci, cluster in enumerate(clusters, 1):
                    cluster_text = " | ".join(cluster["items"][:3])
                    ...
                    try:
                        ...
                        if result.get("deepens", False):
                            ...
                            local_deepened.append(refined)
                    except Exception:
                        pass
                
                current_items = noise
            
            return local_deepened
        
        for cycle in range(1, deepen_depth + 1):
            print(f"\n{'='*80}")
            print(f"🔄 DEEPENING CYCLE {cycle}/{deepen_depth} — KB + DBSCAN + LLM Validation (PARALLEL)")
            print(f"{'='*80}")
            
            if not kb_loaded or not kb_items:
                print(f"      ⏸️  No KB data. Skipping.")
                break
            
            # Process hypotheses in parallel
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_PARALLEL_LLM_CALLS) as executor:
                futures = {executor.submit(process_hypothesis_deepening, hyp): hyp for hyp in approved_hypotheses}
                for future in concurrent.futures.as_completed(futures):
                    try:
                        result = future.result()
                        print(f"      ✅ Processed hypothesis, found {len(result)} deepenings")
                    except Exception as e:
                        print(f"      ⚠️ Error: {e}")
            
            # Collect all deepened hypotheses
            all_deepened = []
            for hyp in approved_hypotheses:
                hyp_words = set(hyp.lower().split())
                relevant_items = [item for item in kb_items if len(hyp_words & set(item.lower().split())) >= 2]
                if len(relevant_items) < 3:
                    relevant_items.extend(_random.sample(kb_items, min(5, len(kb_items))))
                ...
                for r in range(1, 5):
                    ...
                    for cluster in clusters:
                        ...
                        try:
                            ...
                            result = json.loads(completion.choices[0].message.content)
                            if result.get("deepens", False):
                                all_deepened.append(result.get("refined_hypothesis", hyp))
                        except Exception:
                            pass
                    current_items = noise
            
            if all_deepened:
                approved_hypotheses = all_deepened
                print(f"\n   📊 Cycle {cycle} produced {len(all_deepened)} deepened hypotheses")
            else:
