...
MAX_PARALLEL_SOCRATIC = 3  # Parallel Socratic question processing
...
def run_questioning_engine(original_problem: str) -> Dict[str, Any]:
    ...
    # Step 2: Iterative Socratic refinement for each hypothesis (PARALLEL OPTIMIZATION)
    all_socratic_responses = []
    socratic_log = []
    refined_hypotheses = []
    
    def process_hypothesis_parallel(args):
        """Process a single hypothesis - runs in parallel"""
        i, hypothesis = args
        try:
            local_client = Groq()
            final_hypothesis, responses, iterations_used = refine_hypothesis_socratic(
                local_client, hypothesis, max_iterations=3
            )
            return (i, hypothesis, final_hypothesis, responses, iterations_used)
        except Exception as e:
            print(f"   ⚠️ Error processing hypothesis #{i}: {e}")
            return (i, hypothesis, hypothesis, [], 0)
    
    # Process hypotheses in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_PARALLEL_SOCRATIC) as executor:
        futures = {executor.submit(process_hypothesis_parallel, (i, hyp)): (i, hyp) 
                   for i, hyp in enumerate(approved_hypotheses, 1)}
        
        for future in concurrent.futures.as_completed(futures):
            i, hypothesis, final_hypothesis, responses, iterations_used = future.result()
